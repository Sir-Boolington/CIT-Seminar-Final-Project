      ...
      <div className="relative z-10 flex items-center justify-center min-h-[calc(100vh-65px)] px-7">
        <div className="w-full max-w-md text-center">
          ...
          <p className="text-sm text-ts-text3 mb-6 max-w-sm mx-auto">
            Face a timed sequence of cybersecurity scenario cards. Identify the threat and pick the safest course of action. Choose your difficulty to begin.
          </p>
          <div className="flex gap-3 justify-center mb-6">
            {[
              { level: 'easy', name: 'Beginner', desc: 'No time limit, full hints', color: 'border-green-500/30 hover:border-green-500/60 text-ts-green' },
          ...
          <div className="flex justify-center gap-2 flex-wrap">
            ...
            <span className="text-[10px] px-2.5 py-1 rounded-full bg-ts-surface border border-ts-border text-ts-text3 opacity-50">Social (stretch)</span>
            <span className="text-[10px] px-2.5 py-1 rounded-full bg-ts-surface border border-ts-border text-ts-text3 opacity-50">Vishing (stretch)</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative z-10 max-w-2xl mx-auto px-7 py-7">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[11px] text-ts-text3">Difficulty: {difficulty === 'easy' ? 'Beginner' : difficulty === 'medium' ? 'Intermediate' : 'Expert'}</span>
      </div>
      <div className="w-full h-1 bg-ts-surface rounded-full mb-6"><div className="w-[20%] h-full bg-ts-accent rounded-full" /></div>
      <div className="bg-ts-surface border border-ts-border rounded-xl p-6 mb-6">
        ...
        <h3 className="text-base font-medium text-ts-text mb-3">Urgent: Verify your account credentials</h3>
        <div className="bg-ts-bg border border-ts-border rounded-lg p-4 mb-4">
          ...
          <p className="text-xs text-ts-text2 leading-relaxed">
            ...
            Please verify your credentials immediately by clicking the link below to prevent account suspension.<br /><br />
            ...
            Best regards,<br />IT Security Team
          </p>
        </div>
        <p className="text-xs text-ts-text3 mb-4">What is the best course of action?</p>
        <div className="flex flex-col gap-2">
          {['A) Click the link and verify your credentials', 'B) Forward the email to your real IT department', 'C) Reply asking for more details', 'D) Delete the email and ignore it'].map((opt) => (
            <button key={opt} disabled className="text-left px-4 py-3 bg-ts-bg border border-ts-border rounded-lg text-xs text-ts-text2 hover:border-ts-border-h transition-colors disabled:opacity-60">{opt}</button>
          ))}
        </div>
      </div>
      <p className="text-center text-[11px] text-ts-text3 italic">Scenario engine coming soon — scenario data + scoring pipeline in progress</p>
      <div className="flex justify-between mt-6">
        <button onClick={() => setDifficulty(null)} className="text-[11px] text-ts-text3 border border-ts-border rounded-md px-4 py-2 hover:border-ts-border-h transition-colors">Quit session</button>
        <button disabled className="text-[11px] text-white bg-ts-accent rounded-md px-4 py-2 opacity-50">Submit answer</button>
      </div>
    </div>
  );
}
